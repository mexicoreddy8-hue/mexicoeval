...
import { listLocations, createLocation } from "@/lib/db";
...
export default function Locations() {
  ...
  async function save() {
    ...
    if (SUPABASE_READY) {
      try { await createLocation(form); await reload(); toast("Location added"); }
      catch { toast("Could not save", "alert-triangle"); }
    } else { toast("Connect Supabase to save", "info"); }
    setOpen(false); setForm({ color: COLORS[0] });
  }
    ...
    <Shell portal="admin" title="Locations" sub="Evaluation sites across the program">
      <div className="card">
        <div className="card-head">
          ...
          <button className="btn btn-pri" onClick={() => setOpen(true)}><Icon name="plus" size={16} /> Add Location</button>
        </div>
        <div className="card-pad" style={{ padding: rows.length ? 0 : undefined }}>
          {rows.length === 0 ? (
            <EmptyState icon="map-pin" title="No locations yet"
              ...
              action={<button className="btn btn-pri" onClick={() => setOpen(true)}><Icon name="plus" size={16} /> Add Location</button>} />
          ) : (
            <div className="tbl-wrap"><table className="tbl">
              ...
              <tbody>{rows.map((l) => (
                <tr key={l.id}>
                  ...
                  <td><button className="btn btn-icon btn-xs btn-ghost"><Icon name="pencil" size={14} /></button></td>
                </tr>
              ))}</tbody>
            </table></div>
          )}
